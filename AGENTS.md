...
- A pre-commit hook (husky, installed by `npm install` via `prepare`) runs `eslint --fix` and `prettier --write` on staged files through lint-staged. Fix what it reports rather than bypassing it with `--no-verify`. It doesn't typecheck; `npm run check` and CI do.
...
- Write source under `src/` as `.ts`/`.tsx`. `allowJs` is off, so only the config files (`gatsby-config.js`, `gatsby-node.js`, `eslint.config.js`, `scripts/`) stay JavaScript, plus `src/tools/pagination.js`, which `gatsby-node.js` requires and the templates import through `pagination.d.ts`.
- Keep `gatsby-node` as `gatsby-node.js`. Gatsby compiles a `.ts` version into `.cache/compiled/` before `gatsby-adapter-netlify` restores its build cache over `.cache`, so on Netlify the compiled file disappears, `createPages` silently does nothing, and every article, journal and tag page is missing (the build still succeeds). This only shows up on Netlify, not locally. `npm run verify:build` (run after `build`, and in CI) compares the pages in `public/` with the article count from Kontent, but CI can't reproduce the Netlify cache problem itself, so check the deploy preview when touching `gatsby-node.js`. `gatsby-ssr.tsx` is bundled by webpack and isn't affected.
- Keep `import React` (or `import * as React`) in every `.tsx` file that uses JSX. Gatsby's Babel preset uses the classic JSX runtime, so removing it breaks the build with `React is not defined` while rendering pages, and only `gatsby build` shows it, not `tsc`. `tsconfig` uses `"jsx": "react"` to match.
- `.env` holds Kontent credentials and is git-ignored. Never commit it or print its contents.
- `npm test` runs both suites. `npm run test:unit` runs Vitest unit tests (`src/**/*.test.ts(x)`, next to the code they cover; no build needed). `npm run test:e2e` runs the Playwright smoke tests in `e2e/` against the production build (run `npm run build` first; it starts `gatsby serve` itself). They cover page loads with a clean console, code highlighting, tag and pagination navigation, and the theme toggle. They don't replace looking at UI changes in a browser, as described below.
